...
while os.path.exists(Sample_path):
    ...
    while os.path.exists(TS_path):
        ...
        while os.path.exists(file_path):
            ...
            H_array_Neck.append(np.mean(Neck_H))
            H_array_Sphere.append(np.median(Sphere_H))
            #H_array_Neck.extend(Neck_H)
            #H_array_Sphere.extend(Sphere_H)
...
# Number of arrays
n1 = len(H_array_Neck_Sample)

# Find the maximum length of the arrays
max_length = max(len(arr) for arr in H_array_Neck_Sample)

# Pad arrays with NaN to make them the same length
padded_arrays = [np.pad(arr, (0, max_length - len(arr)), constant_values=np.nan) for arr in H_array_Neck_Sample]

# Create a dictionary with the padded arrays
data_dict = {'E_delta2': padded_arrays[0], 'E_ko': padded_arrays[1], 'E_wt': padded_arrays[2]}

df = pd.DataFrame(data_dict)
df.to_csv('H_Neck.csv', index=False)


# Number of arrays
n1 = len(H_array_Sphere_Sample)

# Find the maximum length of the arrays
max_length = max(len(arr) for arr in H_array_Sphere_Sample)

# Pad arrays with NaN to make them the same length
padded_arrays = [np.pad(arr, (0, max_length - len(arr)), constant_values=np.nan) for arr in H_array_Sphere_Sample]

# Create a dictionary with the padded arrays
data_dict = {'E_delta2': padded_arrays[0], 'E_ko': padded_arrays[1], 'E_wt': padded_arrays[2]}

df = pd.DataFrame(data_dict)
df.to_csv('H_Sphere.csv', index=False)
...
bp1 = plt.boxplot(H_array_Neck_Sample, positions=positions1, widths=0.3, patch_artist=True,
...
bp2 = plt.boxplot(H_array_Sphere_Sample, positions=positions2, widths=0.3, patch_artist=True,
...
for i, data in enumerate(H_array_Neck_Sample):
...
for i, data in enumerate(H_array_Sphere_Sample):
    x_jitter = np.random.normal(0, 0.05, size=len(data))  # Add small horizontal jitter
...
plt.title('Mean Curvature per VLP of the neck and of the sphere')
plt.xticks(xtick_positions, labels=[r'E$\Delta$H2',r'E knockout',r'E wildtype'])
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig('Plots/Neck_Sphere/Boxplot_Neck_Sphere_H_compare.pdf', format='pdf')
plt.clf()
...
pairwise_results = {}
for i, j in combinations(range(len(H_array_Neck_Sample)), 2):
    t_stat, p_value = ttest_ind(H_array_Neck_Sample[i], H_array_Neck_Sample[j], equal_var=True)
...
for pair, (t_stat, p_value) in pairwise_results.items():
    ...
    # Perform pairwise t-tests
pairwise_results = {}
for i, j in combinations(range(len(H_array_Sphere_Sample)), 2):
    t_stat, p_value = ttest_ind(H_array_Sphere_Sample[i], H_array_Sphere_Sample[j], equal_var=True)
    pairwise_results[f"Sample {i+1} vs Sample {j+1}"] = (t_stat, p_value)
