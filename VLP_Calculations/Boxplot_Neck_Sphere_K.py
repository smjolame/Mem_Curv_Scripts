...
while os.path.exists(Sample_path):
    ...
    while os.path.exists(TS_path):
        ...
        while os.path.exists(file_path):
            ...
            K_list = [Neck_K, Sphere_K]
            K_array.append(K_list)
...
num_samples = len(K_array_Sample)

# Calculate the maximum number of pairs across all samples
max_pairs = max(len(sample) for sample in K_array_Sample)
...
for i, (ax, sample) in enumerate(zip(axs, K_array_Sample)):
...
plt.savefig('Plots/Neck_Sphere/Boxplot_Neck_Sphere_K.pdf', format = 'pdf')