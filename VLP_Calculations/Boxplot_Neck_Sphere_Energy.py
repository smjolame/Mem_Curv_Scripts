...
while os.path.exists(Sample_path):
    ...
    while os.path.exists(TS_path):
        ...
        while os.path.exists(file_path):
            ...
            Energy_list = [Neck_Energy, Sphere_Energy]
        ...
        TS +=1
        TS_path = f'Results/Sample_{Sample}/TS00{TS}'
    Energy_array_Sample.append(Energy_array)
...
num_samples = len(Energy_array_Sample)

# Calculate the maximum number of pairs across all samples
max_pairs = max(len(sample) for sample in Energy_array_Sample)
...
for i, (ax, sample) in enumerate(zip(axs, Energy_array_Sample)):
    ...
    ax.set_ylabel(r"Energy $ \:/\: \mathrm{k_BT/nm^2}$")
...
# Add a title
fig.suptitle('Comparison between neck- (left) and sphere- (right) energy within each sample')

plt.savefig('Plots/Neck_Sphere/Boxplot_Neck_Sphere_Energy.pdf', format = 'pdf')
